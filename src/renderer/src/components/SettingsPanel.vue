    ...
    <section class="diagnostics-card" aria-labelledby="diagnostics-title">
      ...
      <ol v-else class="diagnostics-list">
        <li v-for="entry in diagnostics.slice(0, 20)" :key="entry.id">
          ...
          <span>
            {{ entry.event || entry.operation || '—' }}<template v-if="entry.requestContext"> · {{ entry.requestContext }}</template><template v-if="entry.attempt"> · 第 {{ entry.attempt }} 次</template> · {{ entry.status ?? '—' }} · {{ entry.contentType || '无类型' }} · {{ entry.durationMs }} ms<template v-if="entry.queueMs !== undefined"> · 排队 {{ entry.queueMs }} ms</template><template v-if="entry.upstreamMs !== undefined"> · 上游 {{ entry.upstreamMs }} ms</template><template v-if="entry.responseHeadersMs !== undefined"> · 响应头 {{ entry.responseHeadersMs }} ms</template><template v-if="entry.responseBytes !== undefined"> · 已读 {{ formatBytes(entry.responseBytes) }}</template>
          </span>
