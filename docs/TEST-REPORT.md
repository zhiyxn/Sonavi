...
- 文案回归：failure-first 更新 3 个断言后，旧实现有 2 项按预期失败；实现后 `app-shell` 1 文件/11 项、lint、三组 typecheck、32 文件/194 项全量测试与生产构建通过。构建仅有既有 Zod PURE 注释位置提示；当前运行窗口需重启后目视新文案。
- 未验证：macOS Intel 菜单栏、macOS arm64 菜单栏与新安装包仍需实际重启验证。本轮未生成新安装包、未发布、未上传、未创建提交。
