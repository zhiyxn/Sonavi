...
## P12 流程重走：当前源码 macOS Intel x64（2026-09-19）

- 基线：`main` / `67126ea`；工作树开始时干净。没有回退旧 P12 起点，因此包含 P13～P15 的全部后续修复。
- 环境：macOS 13.7.8 Intel x64、Node.js 22.19.0、npm 10.9.3、Electron 44.3.0。
- `npm run lint`：通过，0 warning。
- `npm run typecheck`：node、web、test 三组通过。
- `npm test`：29 文件/172 项通过。
- `npm run build`：通过；只出现既有 Zod PURE 注释位置提示。
- `npm run test:e2e`：受限沙箱内第一次因 Electron 进程无法启动而失败；在获准使用本机桌面环境后同一命令完整通过。启动样本 1504 ms；20 轮切页内存增量 55,792 KiB，媒体请求增量 0。
- `npm run build:mac:x64`：受限沙箱内因 `github.com` DNS 不可用而失败；获准联网取得缺失 Electron 缓存后成功生成未签名 DMG，没有发布或上传。
- `npm run verify:package -- mac-x64`：通过。DMG 135,241,299 字节，SHA-256 `5a37c6579b978d03c17607f9528d2fb05018dc14914c7cbb634929b9a24b17ec`；应用架构 x64、App ID `com.sonavi.desktop`、版本 `0.1.0-rc.4`、最低 macOS 13.0、签名状态 `unsigned`、ASAR 2,117,420 字节。
- `npm run test:e2e:package -- mac-x64`：完整通过。启动样本 1189 ms；20 轮切页内存增量 52,652 KiB，媒体请求增量 0。
- 已打开 `release/0.1.0-rc.4/mac/Sonavi.app` 进入人工矩阵；尚未把启动动作计入任何真实用例结果。
- 用户确认应用自动恢复加密账号并进入真实主界面；本轮 macOS Intel `A-01`、`A-06`、`UI-01` 为 `PASS`。该证据没有记录服务器地址、账号或凭据。
- 用户确认 `UI-02`、`UI-03`、`LIB-01`、`LIB-03`、`LIB-05` 均为 `PASS`：Logo 外链、栏目标题/按钮光标、首页与专辑独立分页、艺术家列表首次显示与缓存恢复、显式提交搜索均符合预期。
- `LIB-07` 为 `FAIL`：从搜索或收藏结果打开专辑都会错误激活“专辑”，返回进入全部专辑列表。已登记为 `P12R-MA-001`；没有网络错误，也未记录搜索词或资源身份信息。
- `LIB-02`、`LIB-04` 为 `PASS`：专辑详情内部滚动与返回位置、艺术家查询缓存/手动刷新均符合预期。
- `LIB-06` 的既有 `PASS` 已撤销并改为 `FAIL`：用户明确要求搜索结果中的艺术家详情保留“搜索”来源，而当前实现无条件激活“艺术家”。已登记为 `P12R-MA-002`，没有记录搜索词或资源身份信息。
- `PLAY-01`、`PLAY-02`、`PLAY-04`、`PLAY-05`、`PLAY-06`、`UI-04` 为 `PASS`：真实物理听音、暂停/继续、原始流前后 seek、上一首/下一首与快速连续切歌、队列定位/样式/关闭交互、主按钮/当前封面和播放器布局均符合预期。
- `PLAY-07`、`PLAY-08`、`PLAY-09` 为 `PASS`：自然播放/循环/随机、重复歌曲与队列修改、逐曲 scrobble 均符合预期。
- `PLAY-03` 为 `FAIL`：保存“MP3 兼容转码”后，从缓存专辑重新创建队列仍显示“原始音频”；手动刷新专辑后再次播放正确显示“兼容转码”。已登记 `P12R-MA-003`，该对照确认播放设置变化后携带旧媒体计划的查询缓存未失效；截图没有复制进仓库或记录歌曲身份信息。
- `PLAY-11` 为 `FAIL`：同步歌词自动居中时可见滚动条反复闪动；同步高亮、seek 定位、普通歌词和无歌词状态均正常。已登记 `P12R-MI-001`；可以隐藏视觉滚动条而保留滚动能力，尚未修改代码。
- 四项修复（待真机复验）：搜索/收藏专辑保留来源并返回原页面；搜索/收藏艺术家及其专辑保持来源并逐级返回；播放策略保存后重取所有缓存媒体查询但不停止当前播放/清空队列；歌词保留滚动能力并隐藏视觉滚动条。
- failure-first：修复前 `app-shell`、`network-policy`、`interaction-style` 定向运行有 5 项按预期失败；实现后 3 文件/23 项通过。
- 用户补充验收：保存兼容转码后优先切换当前歌曲；如果服务器不支持按当前进度换流，至少下一曲必须转码。新增受限队列播放句柄刷新 IPC；不传递凭据、URL 或未校验对象，main 只接受会话 ID 与最多 1000 条受 schema 限制的队列元数据。
- 新增回归：播放器测试覆盖支持 `transcodeOffset` 时从当前进度换流，以及不支持时保持当前流、下一曲转码；应用外壳测试覆盖设置保存触发队列刷新、缓存重取且不停止播放器。新增前 2 项 failure-first 失败，实现后定向 2 文件/35 项通过。
- 完整自动验证：`npm run lint`、`npm run typecheck`、`npm test`（29 文件/177 项）、`npm run build` 与获准后的 `npm run test:e2e` 均通过。E2E 直接确认只修改播放策略后当前播放器标签切换为“兼容转码”；首次包含新断言的完整运行在后段暂停队列持久化等待处出现一次时序失败，同命令立即完整复跑通过，最终源码冒烟启动样本 1124 ms。
- 修复后 macOS x64 包：`npm run build:mac:x64` 通过；`npm run verify:package -- mac-x64` 通过，DMG 135,242,879 字节，SHA-256 `cf4f8d7ef6c98a4c16ae732b5ec961df4e4f50fa989cd99bca753efbfc762efb`，应用 x64、最低 macOS 13.0、未签名、ASAR 2,125,527 字节。
- `npm run test:e2e:package -- mac-x64` 完整通过：启动样本 1137 ms，20 轮切页内存增量 57,736 KiB、媒体请求增量 0；连接、音乐库、播放/seek、歌词、收藏/歌单、转码、诊断、生命周期、恢复与凭据边界均通过 fixture 冒烟。
- 结论：`P12R-MA-001`、`P12R-MA-002`、`P12R-MA-003`、`P12R-MI-001` 从 `OPEN` 转为 `RETEST`；`LIB-06`、`LIB-07`、`PLAY-03`、`PLAY-11` 仍保留此前 `FAIL`，直到新包真实服务器/物理听音复验通过。
- `SET-01`、`SET-02`、`SET-03`、`SET-04`、`UI-05`、`LIB-08` 为 `PASS`。本机真实应用自动操作覆盖缓存确认的取消/执行、清理后封面重新加载、封面压力下播放进度持续、断开取消/确认、空表单安全重连、设置跨进程/重连保持、专辑诊断上下文和通知右上关闭。
- `SET-05` 实时隐私检查通过：诊断非空且不包含服务器值或敏感字段名；schema v4 已有自动回归证据。本轮没有控制原生保存对话框生成真实导出文件，因此人工导出部分为 `BLOCKED`。
...
- 该阶段当时未验证真实服务器的 MIME、MP3/AAC/M4A/FLAC/Opus/Ogg 实机标签与听音矩阵；2026-09-20 用户已确认 `PLAY-12` 为 `PASS`。本结果不替代未单独执行的平台发布验收；本阶段没有重新生成安装包，也没有发布或上传。
...
- 范围：搜索表单对齐、结果区布局、显式分页，以及首页/全部专辑/艺术家详情/搜索/收藏五类专辑列表的歌曲数量；不修改协议、main、preload、凭据、媒体流或服务端数据。
- failure-first：新增专辑元数据与搜索布局测试后，旧实现的四类组件覆盖五个入口时均缺少歌曲数量；完成首轮显示后再加入分页断言，旧滚动续载实现按预期找不到 `pagination-next`。
- 定向回归：`album-list-metadata` 5/5、`search-panel` 2/2 通过；分页交互验证第二页使用 offset 25、size 25，DOM 不保留第一页歌曲。长歌手名回归同时验证歌曲数量位于独立的非收缩元素中；修复后 lint、三组 typecheck 与该定向文件 5/5 再次通过。
- `npm run lint`：通过，0 warning。
- `npm run typecheck`：node、web、test 三组通过。
- `npm test`：31 文件/186 项通过。
- `npm run build`：通过；仅有既有 Zod PURE 注释位置提示。
- `npm run test:e2e`：Windows 源码 Electron 完整通过；实测搜索输入框/按钮同高同顶、歌曲区与发现区同起点并排、搜索专辑显示 3 首歌曲、单页下一页禁用及 960×640 无应用级横向溢出。启动样本 515 ms，20 轮切页内存增量 29,892 KiB、媒体请求增量 0。
- 未验证：真实大型音乐库的多页搜索、五个专辑入口人工可读性、macOS Intel x64、macOS arm64 与新安装包。本轮未发布、未上传、未创建提交。

## P19 慢响应与封面队列治理（2026-09-20）

- 证据：用户提供的 schema v4 文件达到 200 条上限。18 次 API 中 13 次为正文超时，均保留 HTTP 200、JSON、2.2～5.5 秒响应头耗时和 0 正文字节；没有 4xx/5xx、认证、证书或一般网络错误。搜索 8 次超时呈两组默认四次尝试，收藏三次超时后第四次成功。181 条封面均成功，但旧总时长包含调度等待，最长约 18 分钟，不能直接当作单请求上游耗时。
- failure-first：搜索/收藏各观察到 4 次调用；同资源封面 URL 不复用；封面诊断缺少排队/上游字段；schema 仍为 v4；正文超时提示不准确；延迟封面组件不存在。6 个文件共 8 项按预期失败。
- 实现：搜索/收藏 `retry: false`；列表封面用 240px 预加载范围和停用清理；同会话/资源复用有界加密封面 URL；schema v5 增加 `queueMs/upstreamMs`；响应头后正文超时使用专门错误与建议。没有新增 IPC、服务器写入、URL/身份字段或媒体正文记录。
- 定向回归：6 文件/62 项通过。首次全量 32 文件/192 项中仅 2 条旧测试仍查找立即出现的 `<img>`，改为验证延迟组件及其 loading/decoding/fetchpriority 后相关 2 文件/12 项通过；没有放宽产品断言。
- `npm run lint`：通过，0 warning。
- `npm run typecheck`：node、web、test 三组通过。
- `npm test`：32 文件/193 项通过；其中新增用例验证 KeepAlive 页面停用会移除封面请求，重新激活后继续等待进入预加载区域。
- `npm run build`：通过；仅有既有 Zod PURE 注释位置提示。
- `npm run test:e2e`：前两次分别因拆分歌手名/歌曲数后的旧精确文本定位、诊断 UI 只显示最近 20 条而失败；修正测试定位后第三次 Windows 源码 Electron 完整通过。最终启动样本 505 ms，20 轮切页内存增量 24,540 KiB、媒体请求增量 0；直接验证封面诊断同时含数字 `queueMs/upstreamMs`。
- Windows 真实服务人工复验：用户确认 `LIB-08` / `SET-05` 为 `PASS`。schema v5 文件共 106 条、44,613 字节，SHA-256 为 `6ea1c922a434ed0758f2ac0d60974229220e14af15e60e43bdc22f3753eab400`；94 条封面全部为 200/无错误，总耗时平均 2.63 秒、P95 4.27 秒、最大 4.99 秒，94 条均含 `queueMs/upstreamMs`。导出内容未发现禁止字段、HTTP URL、认证查询标记、认证请求头、资源身份或响应正文；用户同时确认封面加载期间界面可操作且播放不中断。
- 未验证：本文件没有 `search3` / `getStarred2`，不能证明搜索/收藏失败时每次只产生一个 API 请求；macOS Intel x64、macOS arm64、新安装包及服务端/反向代理同时间段日志仍未验证。本轮未发布、未上传、未创建提交。

## P20 搜索纵向布局与独立分页（2026-09-20）

- 范围：搜索结果排列、专辑/歌曲独立页码、搜索请求与返回 schema；不新增 IPC 通道、服务器写入、凭据、媒体流或私有 Navidrome API。
- failure-first：更新搜索面板断言后，旧实现仍传单一 offset、只有一个分页器，2 项按预期失败。
- 实现：结果按艺术家、专辑、歌曲上下排列；专辑与歌曲分页器分别改变 `albumOffset` / `trackOffset`，main 将后者映射到公共 `search3.songOffset`。一次请求同时取得当前组合，艺术家固定 offset 0；返回结构分别提供 `albumHasMore` / `trackHasMore`。
- 定向回归：相关 6 文件/57 项通过；新增 LibraryService 用例验证偏移量映射和两个末页状态后，独立分页 2 文件/7 项通过。
- `npm run lint`：通过，0 warning。
- `npm run typecheck`：node、web、test 三组通过。
- `npm test`：32 文件/194 项通过。
- `npm run build`：通过；仅有既有 Zod PURE 注释位置提示。
- `npm run test:e2e`：Windows 源码 Electron 完整通过；根据用户截图新增 20px 分页上间距后，几何断言确认专辑网格/歌曲列表与各自分页器均至少相隔 20px，同时确认搜索纵向顺序、两个单页下一页禁用、专辑歌曲数量及 960×640 无横向溢出。启动样本 706 ms，20 轮切页内存增量 24,172 KiB、媒体请求增量 0。
- 人工复验：2026-09-20 用户确认 `LIB-09` 为 `PASS`，覆盖真实大型音乐库的搜索纵向布局、专辑/歌曲独立分页、分页间距与末页状态，以及五类专辑入口的歌曲数量。
- 未验证：未单独执行的 macOS Intel x64、macOS arm64 与新安装包仍保持未验证；本轮未发布、未上传、未创建提交。

## P21 系统托盘安全重启（2026-09-20）

- 人工结论：用户确认 `PLAY-12`、`LIB-08`、`SET-05`、`LIFE-06`、`LIB-02` 与 `LIB-07` 均为 `PASS`；Windows 当前源码人工清单至此全部完成，汇总为 37 PASS、0 FAIL、0 BLOCKED、0 NOT TESTED。
- 范围：只在共享托盘/菜单栏模板中增加“重启 Sonavi”，并调用既有 `DesktopIntegrationController.requestRestart()`；不新增 IPC、preload API、renderer 能力或任意进程参数。
- failure-first：新断言首先得到托盘菜单少一项的预期失败；实现后 `desktop-integration` 1 文件/5 项通过，同时验证重启和退出为独立动作。
- `npm run lint`：通过，0 warning。
- `npm run typecheck`：node、web、test 三组通过。
- `npm test`：32 文件/194 项通过。
- `npm run build`：通过；仅有既有 Zod PURE 注释位置提示。
- `npm run test:e2e`：Windows 源码 Electron 完整通过；启动样本 608 ms，20 轮切页内存增量 47,192 KiB，媒体请求增量 0。
- 首轮开发预览：从 `electron-vite dev` 实例点击托盘重启后窗口空白；检查时 5173 无监听，新进程仍使用已失效的开发 renderer URL。该证据定位为测试启动方式问题，不以此冒充打包路径失败。
- Windows 人工复验：改用已构建、不依赖 5173 的 `electron .` 入口后，用户确认托盘重启后主界面正常显示与操作，真实播放正常；进程检查只存在一个 Sonavi Electron 主进程。Windows 当前构建入口记为 `PASS`。
- 设置页人工复验：用户确认取消分支不会重启，确认分支可安全重启并恢复可操作主界面；`LIFE-06` 在 Windows 记为 `PASS`。
- 音乐库收口复验：用户确认 `LIB-02` / `LIB-07` 为 `PASS`，覆盖专辑详情内部滚动与返回位置，以及六个数据页各自的位置、内容和查询复用。补充路径确认两级来源上下文正确；用户指出收藏上下文中“返回艺术家详情”易被理解为跳到顶级艺术家栏目，因此改为“返回艺术家专辑”，返回目标不变。
- 文案回归：failure-first 更新 3 个断言后，旧实现有 2 项按预期失败；实现后 `app-shell` 1 文件/11 项、lint、三组 typecheck、32 文件/194 项全量测试与生产构建通过。构建仅有既有 Zod PURE 注释位置提示；重启后用户目视确认“返回收藏”→“返回艺术家专辑”及逐级返回未发现问题。
- macOS Intel 补充证据：用户确认已在 Intel 真机试用当前版本且未发现问题；由于未逐项点名执行完整矩阵，本报告只记实机烟测通过，不外推每个用例。
...
- failure-first：新增单实例生命周期测试后，旧代码因不存在单实例模块而稳定失败，证明此前没有相应行为边界。
- 定向回归：4 项通过，覆盖次实例退出且不注册监听、主实例重复唤醒、初始化阶段多次事件合并和退出时移除监听器。
- `npm run lint`：通过，0 warning。
- `npm run typecheck`：node、web、test 三组通过。
- `npm test`：33 文件/198 项通过。
- `npm run build`：通过；只有既有 Zod PURE 注释位置提示。
...
- 候选范围：P26 默认专辑、音乐栏目、三页内嵌搜索和艺术家服务端分页；P27 艺术家自然滚动、三页搜索条吸顶和专辑内容间距。rc.5 的单实例、多服务器、歌词对比度与切换确认同时保留。
- 本地闸门：Node.js 22.19.0 下 `npm run lint`、三组 `npm run typecheck`、35 文件/205 项 `npm test -- --run`、`npm run build` 和 `npm run test:e2e` 全部通过。
