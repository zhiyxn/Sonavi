...
连接与媒体请求使用每次请求独立 salt 的 token 认证，不发送明文密码参数。JSON 客户端保留 1 MiB 安全上限；媒体响应不走 JSON 缓冲，而由 `sonavi-media` 直接流式传递。P08 原始模式发送 `format=raw`；兼容模式发送 `format=mp3`、受限 `maxBitRate` 和 `estimateContentLength=true`。自动模式只对已知 Chromium 媒体类型优先原始流，并预生成一个兼容回退句柄；解码错误最多回退一次，未知类型直接选择兼容转码。
