...
| 专辑与详情 | `getAlbumList2` / `getAlbum` | P03 | 已实现；受控 fixture 与 Electron IPC 通过，真实服务器待验证 |
| 封面与播放 | `getCoverArt` / `stream` | P03 | 已实现不透明句柄、流式响应和 Range；真实服务器/物理听音待验证 |
...
连接与媒体请求使用每次请求独立 salt 的 token 认证，不发送明文密码参数。JSON 客户端保留 1 MiB 安全上限；媒体响应不走 JSON 缓冲，而由 `sonavi-media` 直接流式传递。P03 已覆盖 200/206/416、单段 Range、重定向拒绝和非媒体响应拒绝。自动测试不访问真实服务；真实 Navidrome/OpenSubsonic、转码差异与反向代理组合仍待验证。
