...
状态：P27 艺术家自然滚动与页内搜索条吸顶已通过当前 macOS Intel 自动代码闸门与用户试用；最新候选 `v0.1.0-rc.6` 已通过三目标原生 release gate 并发布为 GitHub Pre-release。真实大型服务边界、正式签名/公证与三目标完整实机发布验收未完成
更新日期：2026-09-25
...
两个平台共用 Navidrome/Subsonic/OpenSubsonic 客户端、认证业务、音乐库、页面内搜索、收藏、歌单、AudioEngine、播放队列、歌词、缓存、设置、组件、导航、页面布局、播放器和交互逻辑。
...
P01 的上述验收项已经完成。P02 已实现公共 Subsonic/OpenSubsonic 连接、认证、能力探测、CredentialStore 加密保存/跨重启恢复/删除、会话退出与错误诊断；P23 在不改变协议客户端的前提下扩展为最多 20 个系统加密账号。用户可从“服务器”页添加、更新、切换或删除非当前账号，连接页也可直接连接已保存账号；renderer 只取得 profile ID、服务器地址与用户名，密码不会回填。P03 实现真实专辑列表/详情、不透明媒体协议和流式播放/暂停/seek。P04 实现枚举播放状态机与内存队列。P05 建立音乐库、艺术家/详情、可取消分页搜索与设置入口；P26 在此基础上移除首页和独立搜索页，默认进入专辑，并将搜索分别收进音乐、专辑、艺术家页面。收藏、歌单、持久化恢复、托盘和桌面集成继续保留既有实现，不能回退假数据。
...
- 首页和独立搜索页已移除；连接成功、恢复连接或重新连接后默认展示专辑页。专辑页空搜索时使用公共 `getAlbumList2` 的 `alphabeticalByName`，每页 30 张；显式输入关键词后改用公共 `search3` 的专辑窗口并独立分页。
- 音乐页使用公共 `search3` 空查询按歌曲 offset/count 浏览，每页 30 首；输入关键词并按 Enter 或点击搜索按钮后仍在本页展示匹配歌曲。歌曲可进入 P04 的共享播放队列。
- 艺术家列表使用公共 `search3` 空查询按艺术家 offset/count 服务端分页，每页 50 位；输入关键词后在同一页面分页搜索。艺术家详情继续使用 `getArtist`。每页列表进入工作区自然文档流，不建立 480px 内部滚动窗口，栏目切换仍恢复工作区滚动位置。
- `search3` 请求为艺术家、专辑、歌曲分别携带 offset/count，只请求当前页面所需类别；返回分别提供下一偏移和 `hasMore`。搜索失败不执行 TanStack Query 默认的三次自动重试，只保留用户明确刷新或重试，并保留过期请求取消、加载、空和错误状态。
- 专辑、艺术家详情、收藏中的每个专辑条目继续展示协议返回并经校验的歌曲数量。已访问数据在当前连接会话内复用，切换栏目不自动整页重取；各数据页只刷新当前列表、分页或详情。
...
- 权限、会话或服务器失败保留已有查询数据；收藏读取失败不自动连续重试，只保留用户明确刷新或原地重试。错误提醒统一使用项目持有的 shadcn-vue Sonner，必要的加载失败页面继续保留原地重试。删除歌单与退出并忘记账号使用项目持有的 shadcn-vue AlertDialog 二次确认；P06 不实现本地离线副本、歌词、播放上报或队列持久化。
...
- 诊断区分 API、封面、原始音频和转码音频，记录脱敏状态、类型、分类、耗时和建议，并限制导出大小；封面进一步区分调度排队与上游读取耗时，API 在已收到响应头后正文超时时给出准确提示。
...
- 封面缓存按账号隔离，单项最多 5 MiB、单账号最多 128 MiB并按 LRU 淘汰；清空只作用当前账号。列表封面只在进入视口附近时发起，页面停用时移除待加载图片以允许浏览器取消请求；同一会话与资源复用有界的不透明封面句柄，减少重复浏览器请求。音频继续流式传输，不加入离线缓存。
...
- 音乐、专辑与艺术家列表页的搜索条在工作区纵向滚动时固定于顶部 12px；搜索条与首行内容至少保留 32px 间距，标题区域仍随页面滚动离开。
