...
P01 的上述验收项已经完成。P02 已实现公共 Subsonic/OpenSubsonic 连接、认证、能力探测、CredentialStore 加密保存/跨重启恢复/删除、会话退出与错误诊断；P23 在不改变协议客户端的前提下扩展为最多 20 个系统加密账号。用户可从“服务器”页添加、更新、切换或删除非当前账号，连接页也可直接连接已保存账号；renderer 只取得 profile ID、服务器地址与用户名，密码不会回填。P03 实现真实专辑列表/详情、不透明媒体协议和流式播放/暂停/seek。P04 实现枚举播放状态机与内存队列。P05 实现首页、全部专辑、艺术家/详情、可取消分页搜索与设置入口；收藏歌单、持久化恢复、托盘和完整桌面集成仍按后续阶段推进，不能回退假数据。
...
- 首页最近添加与全部专辑使用真实、可分页的公共协议数据，不伪造推荐或历史；每页 30 张，统一使用项目持有的 shadcn-vue Pagination 显式翻页。首页、全部专辑、艺术家详情、搜索和收藏中的每个专辑条目都展示协议返回并经校验的歌曲数量。已访问数据在当前连接会话内复用，切换栏目不自动整页重取；各数据页提供只刷新当前列表或详情的明确按钮。
- 艺术家列表和详情来自 `getArtists` / `getArtist`；长列表只渲染可见窗口。
- 搜索使用 `search3`，只在用户按 Enter 或点击搜索按钮时提交；艺术家、专辑、歌曲结果上下排列，专辑与歌曲各自使用项目持有的 shadcn-vue Pagination 和独立 offset 显式翻页，每页最多 25 条，不累积无限滚动结果。一次 `search3` 同时携带两个偏移量，避免首屏拆成两次慢请求；艺术家固定展示首批结果。搜索失败不执行 TanStack Query 默认的三次自动重试，只保留用户明确刷新或重试；同时具备过期请求取消、加载/空/错误状态。切换栏目保留搜索条件、专辑页、歌曲页和结果。
