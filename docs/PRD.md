...
状态：P01～P10 已按序实现到当前发布前代码闸门；`v0.1.0-rc.3`（构建自 `cc33168`，不含 P11 修复）已在 release run `35040657787` 三目标全通过并发布为 GitHub Pre-release，P11 审查修复之后的 `v0.1.0-rc.4`（提交 `acb7c8c`）已在 release run `35063014155` 三目标全通过并发布为 GitHub Pre-release，正式签名/公证/实机发布验收未完成
更新日期：2026-09-16
...
- 首页最近添加与全部专辑使用真实、可分页的公共协议数据，不伪造推荐或历史；每页 30 张，统一使用项目持有的 shadcn-vue Pagination 显式翻页。已访问数据在当前连接会话内复用，切换栏目不自动整页重取；各数据页提供只刷新当前列表或详情的明确按钮。
- 艺术家列表和详情来自 `getArtists` / `getArtist`；长列表只渲染可见窗口。
- 搜索使用 `search3`，只在用户按 Enter 或点击搜索按钮时提交，具备过期请求取消、分页、加载/空/错误/重试状态；切换栏目保留搜索条件和结果，用户可明确刷新当前结果。
...
- 权限、会话或服务器失败保留已有查询数据；错误提醒统一使用项目持有的 shadcn-vue Sonner，必要的加载失败页面继续保留原地重试。删除歌单与退出并忘记账号使用项目持有的 shadcn-vue AlertDialog 二次确认；P06 不实现本地离线副本、歌词、播放上报或队列持久化。
