...
- 搜索使用 `search3`，只在用户按 Enter 或点击搜索按钮时提交；每页以相同 offset/size 获取艺术家、专辑和歌曲，并使用项目持有的 shadcn-vue Pagination 显式翻页，不累积无限滚动结果。宽屏下歌曲区与艺术家/专辑发现区并排，窄屏时歌曲区优先，避免大量专辑把歌曲结果推到页面末尾；同时具备过期请求取消、加载/空/错误/重试状态。切换栏目保留搜索条件、当前页和结果，用户可明确刷新当前页。
