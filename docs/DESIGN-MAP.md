...
当前主机的 P04 截图位于 `artifacts/screenshots/p04-current-platform.png` 与 `p04-macos-x64-package.png`。macOS Intel 已检查专辑详情、队列浮层、播放器、中文/英文字体和控制按钮，无截断或重叠；该目录是本地测试证据，不作为另一平台验收结论。
