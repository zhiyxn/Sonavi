...
- 当前开发窗口已随 main/renderer 变更自动重载；真实服务器复验、Windows 人工、macOS Intel/arm64 和新安装包均未验证。本轮未发布、未推送、未创建提交。下一入口是重新执行 `LIB-08`、`SET-05`，并在同一慢服务上对比新的 schema v5 日志。
...
- 当前开发窗口已热更新。真实大型搜索结果、Windows 人工操作、macOS Intel/arm64 和新安装包未验证；本轮未发布、未推送、未创建提交。下一入口是执行更新后的 `LIB-09`。
