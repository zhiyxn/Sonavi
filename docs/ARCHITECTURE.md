...
- `src/main/`：窗口、安全策略、IPC 注册和后续网络/存储服务；允许使用 Node.js/Electron。
- `src/main/platform/`：最小平台差异。P01 只含原生窗口选项、菜单、快捷键标签和最后窗口生命周期。
- `src/preload/`：唯一 renderer 桥；只暴露 `window.sonavi.application.getInfo()`，不暴露原始 IPC 或 Node 对象。
...
应用信息 IPC 同时执行：主 frame/所属 BrowserWindow 检查、开发 origin 或打包后精确文件路径检查、返回数据 Zod 校验。renderer 再校验返回值。后续所有业务 IPC 复用同级别校验，并增加参数、会话与资源范围校验。

凭据、网络和媒体尚未在 P01 实现。P02 通过统一 CredentialStore 在 main 调用 Electron 44 的 safeStorage；失败时只允许会话内使用，不写明文。API、封面和音频后续共用 Electron Session 网络策略。
