# Sonavi 0.1.0-rc.2 测试版

这是 Sonavi 首个可下载的跨平台候选测试版，供 Windows 11 x64、macOS 13+ Intel x64 和 macOS 13+ Apple Silicon arm64 用户试用。`rc.1` 的 Intel 发布闸门失败且未创建 Release；`rc.2` 保留了失败摘要诊断，并重新执行全部平台闸门。

## 下载

- Windows x64：`Sonavi-0.1.0-rc.2-win-x64.exe`
- macOS Intel x64：`Sonavi-0.1.0-rc.2-mac-x64.dmg`
- macOS Apple Silicon arm64：`Sonavi-0.1.0-rc.2-mac-arm64.dmg`
