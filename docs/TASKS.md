...
- [ ] 推送 `v0.1.0-rc.4`（首个包含 P11 审查修复的候选），重新运行三个 release gate，并确认七个下载附件与 Pre-release 安全说明。
...
P01 已达到验收条件，P02～P10 已按序达到当时可用环境的代码闸门。P11 修复复核允许进入真机测试，当前为 0 Blocker、0 Critical、4 Major；仍不能判定为发布就绪。release run `35040657787` 只证明标签 `v0.1.0-rc.3`（构建自 `cc33168`）当时的三目标自动化闸门，不覆盖其后提交 `557b62d` 的 P11 修复与新 logo，也不替代真实服务器、真实格式、安装、手工桌面行为、最低系统和物理听音。
