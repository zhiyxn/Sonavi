...
- [ ] P12-MI-010：专辑详情返回恢复列表滚动位置已完成并通过定向测试；当前运行包不包含，等待下一统一包真机复验。
- [ ] P12-MI-011：首页、专辑、艺术家、搜索、收藏、歌单及详情切换后恢复各自滚动位置；设置页按用户要求每次回到顶部，退出/更换账号清空记录。最新定向测试 11/11、26 文件/137 项全量测试、lint 与 typecheck 通过，当前运行包不包含，等待下一统一包真机复验。
...
- [ ] P12-MI-013：专辑详情已改为固定外层页面、仅歌曲列表内部滚动并支持键盘焦点；25 文件/135 项全量测试、lint、typecheck 与生产构建通过，当前运行包不包含，等待下一统一包真机复验。
...
- [ ] P12-MI-018：首页、专辑、艺术家、搜索、收藏和歌单已改为保留页面实例并复用当前连接会话查询缓存，切换栏目不再自动整页重取；各页刷新按钮只重取当前列表、分页或详情，写操作与网络/会话变化仍主动失效。28 文件/146 项测试、lint 与 typecheck 通过；未重新构建或启动应用。
...
- [x] 更新产品、架构、决策、测试报告、人工用例和交接记录；真实服务器格式准确性、Windows 11 x64 与 macOS arm64 继续分别标注为未验证。
...
- [ ] 使用真实大型搜索结果复验页码、歌曲可见性及五个专辑入口的歌曲数量；macOS Intel x64、macOS arm64 与对应安装包均未验证。

## P19：慢响应与封面队列治理（代码完成，待真实服务器复验，2026-09-20）
...
- [ ] 使用同一真实服务复验搜索/收藏失败时每次只产生一个 API 请求、离开长列表后封面队列缩短，并导出 schema v5 对照 `queueMs/upstreamMs`；Windows 人工、macOS Intel x64、macOS arm64 与新安装包均未验证。

## P20：搜索纵向布局与独立分页（代码完成，待真实服务器复验，2026-09-20）
...
- [ ] 使用真实大型搜索结果验证两个分页长期交替操作、空末页返回、滚动定位及慢服务表现；Windows 人工、macOS Intel x64、macOS arm64 与新安装包均未验证。
