...
- [x] Electron 冒烟将脱敏失败栈写入 CI Check Summary；不增加自动重试、不删除断言。诊断提交 `54e15df` 的 run `34938572347` 随后三目标全通过，确认先前 Intel 失败为未稳定复现的 runner 时序波动。
- [ ] 推送 `v0.1.0-rc.2`，重新运行三个 release gate，并确认七个下载附件与 Pre-release 安全说明。
