...
## D032：慢服务器下限制读取重试并让封面压力可取消、可归因

- 日期：2026-09-20
- 状态：已接受

真实 schema v4 日志显示 `search3` 与 `getStarred2` 在已经收到 HTTP 200 响应头后正文读取超时；TanStack Query 默认三次重试把单次操作放大为四个连续 12 秒请求。搜索与收藏读取因此明确关闭自动重试，只保留界面中的用户刷新/重试。专辑列表既有的一次自动重试不变，写操作仍不自动重试。

列表封面以 IntersectionObserver 在视口外 240px 范围内才挂载图片，页面停用或组件卸载时移除图片节点；同一会话与资源复用相同加密封面 URL，复用索引每会话最多 10,000 项并随会话撤销清空。音频句柄、流式传输、六并发上游限制和安全校验均不改变。

诊断升级为 schema v5：封面记录纯数字 `queueMs` 与 `upstreamMs`，区分进入调度队列后的等待和取得槽位后的上游耗时。API 已收到响应头后正文超时时使用专门错误类型，只改善用户提示和诊断建议；不记录 URL、查询词、资源 ID、响应正文、账号或凭据。

## D033：搜索纵向排列并让专辑、歌曲独立分页
...
## D037：音乐库浏览与页内搜索统一使用分类型的 `search3` 窗口

- 日期：2026-09-25
- 状态：已接受；取代 D024、D028、D031、D033 中关于首页、独立搜索页和完整艺术家索引的当前 UI 部分

公共 `getArtists` 返回完整索引，没有 offset/count；公共 `search3` 则为艺术家、专辑、歌曲分别提供独立的 offset/count，OpenSubsonic 还明确要求空查询返回全部数据。因此音乐页和艺术家页分别以空查询、仅开启当前类别 count 的 `search3` 窗口实现服务端浏览和分页，非空查询继续复用同一窗口完成页内搜索。艺术家详情仍使用 `getArtist`。专辑页为空查询时保留语义更明确的 `getAlbumList2(alphabeticalByName)`，非空时只请求 `search3` 的专辑窗口。

共享请求和 Zod schema 必须同时携带三类独立 offset/count；未参与页面的类别 count 为 0，至少一类 count 大于 0。返回值分别计算三类下一偏移与 `hasMore`，不根据不存在的总数猜测页数。旧 `getArtists` 能力可保留给兼容代码，但当前 UI 不再用它加载完整索引，也不再依赖其 16 MiB/45 秒特殊边界。

首页和独立搜索页从导航与状态模型中删除，连接后的默认栏目固定为专辑；音乐、专辑、艺术家各自保存输入、已提交查询和页码。现有 AbortSignal/requestId 取消、会话校验、纯文本元数据、不透明媒体句柄、收藏失效和来源详情链全部保留。该决定不新增 IPC 通道、服务端写入或 Navidrome 私有 API；不同服务器对空查询的实际兼容性仍需真实服务验证。

## D038：分页后的艺术家列表使用工作区自然滚动，搜索条统一吸顶

- 日期：2026-09-25
- 状态：已接受；取代 D005/D024 中当前艺术家 UI 仍需内部窗口化滚动的部分

艺术家列表已经由 D037 限制为每页最多 50 项，不再需要为一次性完整索引设计 480px 固定视口和 overscan。当前页完整渲染到普通文档流，`.workspace` 成为唯一纵向滚动容器；应用外壳既有的栏目/详情滚动键负责切换后的恢复。历史 10,000 项窗口化测试只代表旧完整索引方案的证据，不再描述当前 UI。

音乐、专辑和艺术家列表页共用一个吸顶搜索表单样式：`position: sticky`、`top: 12px`、`z-index: 20`，并在结果前保留 32px 下间距。页面标题随工作区滚动离开，搜索操作持续可达；艺术家分页后将搜索表单滚回可见区域。该调整只改变 renderer 布局和滚动状态，不改变查询、IPC、安全、播放或收藏边界。
